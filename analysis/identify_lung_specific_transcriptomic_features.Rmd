...
Barplots for number or DE genes detected for each cell type

Th17 and Treg were not included in the following analyses due to low number of cells in spleen.
...
ggplot(
  ...
  xlab("") + ylab("Numer of DE genes with at least two fold change") + 
...
name_order <- c("common", "Memory_B", "Naive_B", "NK", "CD4_T", "CD8_T")
...
go_tbl <- go_tbl[go_tbl$database != "disease_OMIM", ]
top <- 
  go_tbl %>% 
    filter(FDR < 0.01) %>%
...
go_tbl <- do.call(rbind, go_list)
go_tbl <- go_tbl[go_tbl$database != "disease_OMIM" & go_tbl$k != "common", ]
